...
void renderer::init(SDL_Window* window){
  init_vulkan(window);
  init_cpu_renderer(window_extent.width, window_extent.height);
  init_swapchain();
  init_commands();
...
{
  // for testing but should be remove nonetheless
...
void renderer::init_swapchain(){
  ...
  // needed for cpu
  draw_image.imageFormat = VK_FORMAT_R8G8B8A8_UNORM;
...
/*
    for (int i = 0; i < draw_image.imageExtent.height; i++) {
    	for (int j = 0; j < draw_image.imageExtent.width; j++) {
...
//void renderer::init_cpu_renderer() {
//  //cpu_renderer.clear(color);
//    cpu_renderer.pixels = std::vector<uint32_t>(draw_image.imageExtent.height * draw_image.imageExtent.width, 0xFF000000);
//
//}
//


void renderer::cpu_submit() {
    VkDeviceSize image_size = cpu_rend->config.width * cpu_rend->config.height * 4;
...
void renderer::render(game* game_inst) {
    ...
    // DONE: get the fucking draw_extent in the cpu_renderer 
    // TODO: make imgui work

    cpu_clear(draw_extent.width, draw_extent.height);
    game_inst->render(game_inst, 1.0f);
    cpu_submit(); 
...
void renderer::resize_swapchain(SDL_Window* window) {
    ...
    //draw_image.imageFormat = VK_FORMAT_R16G16B16A16_SFLOAT;
    // needed for cpu
    draw_image.imageFormat = VK_FORMAT_B8G8R8A8_UNORM;
    draw_image.imageExtent = drawImageExtent;
